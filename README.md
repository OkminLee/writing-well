...
"릴리스 노트 좀 써줘"에 딸려온 재료가 이게 전부라고 하자.
...
| | |
|---|---|
| **스킬 없이** | 응답 시간이 340ms에서 90ms로 줄었다. 동료가 말했다. "이제야 쓸 만하네." |
| **writing-well** | 캐시 레이어를 걷어냈다. 체감은 확실히 빨라졌다. `[측정치?]` |

`340ms`도 `90ms`도 재료에 없다. 동료는 아무 말도 한 적이 없다. 그런데 위쪽이 훨씬 잘 읽힌다 — 그래서 지어낸다. 대신 스킬은 글 끝에 이걸 붙인다.
